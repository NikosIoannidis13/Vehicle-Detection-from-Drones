...
if __name__ == '__main__':
    ...
    session_dir = os.path.dirname(annot_dir)          # one level up
    label_dir_name = "Labels_rotated" if is_rotated_box else "Labels_axis_aligned"
    labels_dir = os.path.join(session_dir, label_dir_name)
    os.makedirs(labels_dir, exist_ok=True)            # create once, no error if it exists
    ...
    for fname in file_names[:]:
        ...
        if save_bbox_info:
            out_csv = os.path.join(labels_dir, f"{fname}_{label}.csv")
